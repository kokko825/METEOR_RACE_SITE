...
export const activePlayers = (state) => state.players?.length ? state.players : ["red", "blue"];
export const activeObstacles = (_state) => [];
export const canPlaceObstacle = (_state, _player) => false;
export const obstacleCount = (_state, _player) => 0;
export const nextPlayer = (state, player = state.turn) => {
    const players = activePlayers(state);
    return players[(players.indexOf(player) + 1) % players.length];
};
export const samePos = (a, b) => a.r === b.r && a.c === b.c;
export const distance = (a, b) => Math.max(Math.abs(a.r - b.r), Math.abs(a.c - b.c));
export const orthogonallyAdjacent = (a, b) => Math.abs(a.r - b.r) + Math.abs(a.c - b.c) === 1;
export const viewToBoardPos = (view, size, perspectiveSlot) => {
    const last = size - 1;
    const slot = ((perspectiveSlot % 4) + 4) % 4;
    if (slot === 1)
        return { r: last - view.r, c: last - view.c };
    if (slot === 2)
        return { r: view.c, c: last - view.r };
    if (slot === 3)
        return { r: last - view.c, c: view.r };
    return view;
};
export const boardToViewDelta = (delta, perspectiveSlot) => {
    const slot = ((perspectiveSlot % 4) + 4) % 4;
    if (slot === 1)
        return { r: -delta.r, c: -delta.c };
    if (slot === 2)
        return { r: -delta.c, c: delta.r };
    if (slot === 3)
        return { r: delta.c, c: -delta.r };
    return delta;
};
export const playerName = (player) => player.toUpperCase();
export const meteorName = (size) => (size === "small" ? "小メテオ" : "大メテオ");
export function coreWinner(state, reached) {
    if (reached.includes(state.turn))
        return state.turn;
    const players = activePlayers(state);
    const turnIndex = players.indexOf(state.turn);
    for (let offset = 1; offset <= players.length; offset += 1) {
        const candidate = players[(turnIndex + offset) % players.length];
        if (reached.includes(candidate))
            return candidate;
    }
    return reached[0];
}
export function initialGameState(size = 9, first = "red", playerCount = 2, obstaclesEnabled = false, layoutOffset = 0, botPlayers = [], variant = "classic") {
    if (variant === "team") {
        playerCount = 4;
    }
    if (variant === "item")
...
export function legalMoves(state, player = state.turn) {
    ...
    const maxSteps = state.variant === "item" && (state.boosterMoves?.[player] ?? 0) > 0 ? 2 : 1;
    const moves = [];
    directions.forEach((direction) => {
        for (let step = 1; step <= maxSteps; step += 1) {
            ...
            const legal = target.r >= 0 &&
                target.c >= 0 &&
                target.r < state.size &&
                target.c < state.size &&
                !players.some((candidate) => candidate !== player && samePos(target, state.probes[candidate])) &&
...
export const teamOf = (player) => player === "red" || player === "yellow" ? "sun" : "moon";
function stateKey(state, nextTurn) {
...
export function finishTurn(draft, extraLog) {
    ...
    return {
        ...
        playerTurns,
        repetitions,
        selected: inventory[nextTurn].small > 0
            ? "small"
...
export function applyMove(state, target) {
    ...
    if (picked?.kind === "booster")
        boosterMoves[state.turn] = 2;
    else if ((boosterMoves[state.turn] ?? 0) > 0)
        boosterMoves[state.turn] -= 1;
    if (picked?.kind === "capsule")
        capsuleMeteors[state.turn] += 1;
    const pickedLabel = picked?.kind === "shield"
        ? "シールド"
        : picked?.kind === "booster"
            ? "ブースト"
            : picked?.kind === "capsule"
    ...
    if (target.r === mid && target.c === mid) {
        return {
            ...
            bonusMove: false,
            winner: state.turn,
            message: state.variant === "team"
                ? `${playerName(state.turn)} / ${teamOf(state.turn) === "sun" ? "RED + YELLOW" : "BLUE + GREEN"} TEAM WIN!`
    ...
    const meteorless = state.inventory[state.turn].small +
    ...
    if (state.bonusMove) {
        return finishTurn({ ...state, probes, bonusMove: false, log }, `${playerName(state.turn)}：ボーナス移動完了`);
    }
    const hasPlacement = state.inventory[state.turn].small +
        state.inventory[state.turn].large +
        (state.capsuleMeteors?.[state.turn] ?? 0) >
        0 ||
        canPlaceObstacle(state);
    if (!hasPlacement)
        return finishTurn({ ...state, probes, log }, "配置できるメテオなし");
    return {
        ...state,
        probes,
        phase: "place",
...
export function applyMeteor(state, target, chosenSize, useCapsule = false) {
    const mid = Math.floor(state.size / 2);
    ...
    const probes = Object.fromEntries(PLAYER_ORDER.map((player) => [player, { ...before[player] }]));
    const reached = [];
    activePlayers(state).forEach((player) => {
        const start = before[player];
        const d = distance(start, target);
        const baseSteps = chosenSize === "small" ? (d === 1 ? 1 : 0) : d === 1 ? 2 : d === 2 ? 1 : 0;
        // BOOSTER中は敵の爆風に弱い。ただし自分の爆風を倍化して加速には使えない。
        const steps = (state.boosterMoves?.[player] ?? 0) > 0 && player !== state.turn
            ? baseSteps * 2
            : baseSteps;
        if (!steps)
            return;
        if (state.shield?.[player])
            return;
        ...
        for (let index = 0; index < steps; index += 1) {
            ...
            const blocked = next.r < 0 ||
                next.c < 0 ||
                next.r >= state.size ||
                next.c >= state.size ||
                blockingMeteors.some((meteor) => samePos(meteor, next)) ||
                activePlayers(state).some((candidate) => candidate !== player && samePos(next, before[candidate]));
            if (blocked)
                break;
            position = next;
    ...
    const shieldAfterBlast = {
        ...
        ...Object.fromEntries(activePlayers(state)
            .filter((player) => {
    ...
    const pickupLogs = [];
    activePlayers(state).forEach((player) => {
        if (samePos(before[player], probes[player]))
            return;
        ...
        if (picked.kind === "booster")
            boosterMoves[player] = 2;
        if (picked.kind === "capsule")
            capsuleMeteors[player] += 1;
        const label = picked.kind === "shield"
            ? "シールド"
            : picked.kind === "booster"
                ? "ブースト"
                : "使い捨てメテオ";
    ...
    let next;
    if (reached.length) {
        const winner = coreWinner(state, reached);
        next = {
            ...draft,
            phase: "over",
            winner,
            message: winner === "draw"
                ? "同時到達 — DRAW"
                : state.variant === "team"
                    ? `${playerName(winner)} / ${teamOf(winner) === "sun" ? "RED + YELLOW" : "BLUE + GREEN"} TEAM WIN!`
    ...
    return {
        ...
        pushed: Object.fromEntries(activePlayers(state)
...
export function applyObstacle(state, target) {
    const mid = Math.floor(state.size / 2);
    if (state.phase !== "place" ||
        !canPlaceObstacle(state) ||
        samePos(target, { r: mid, c: mid }) ||
