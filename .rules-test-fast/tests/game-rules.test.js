...
import { applyMeteor, applyMove, applyObstacle, applyPass, activePlayers, boardToViewDelta, coreWinner, finishTurn, initialGameState, legalMoves, samePos, viewToBoardPos, } from "../app/game-rules.js";
for (let slot = 0; slot < 4; slot += 1) {
    ...
    assert.deepEqual(viewToBoardPos({ r: 10, c: 5 }, 11, slot), home, `perspective ${slot} must place its home probe at the bottom`);
}
...
}
{
    ...
    const before = Object.fromEntries(activePlayers(state).map((player) => [player, state.inventory[player].large]));
...
function placementTargets(state) {
    const mid = Math.floor(state.size / 2);
    const targets = [];
    ["small", "large"].forEach((size) => {
        if (!state.inventory[state.turn][size])
            return;
        for (let r = 0; r < state.size; r += 1) {
            for (let c = 0; c < state.size; c += 1) {
                ...
                if ((r === mid && c === mid) ||
                    ...
                    continue;
                const relevant = Math.max(Math.abs(r - state.probes.red.r), Math.abs(c - state.probes.red.c)) <= 2 ||
                    Math.max(Math.abs(r - state.probes.blue.r), Math.abs(c - state.probes.blue.c)) <= 2 ||
                    state.meteors.some((meteor) => Math.max(Math.abs(r - meteor.r), Math.abs(c - meteor.c)) <= 2);
                if (relevant)
                    targets.push({ target, size });
            }
        }
    });
    return targets;
...
function successors(state) {
    if (state.phase === "over")
        return [];
    if (state.phase === "move")
        return legalMoves(state).map((target) => applyMove(state, target));
    return placementTargets(state).map(({ target, size }) => applyMeteor(state, target, size).state);
}
function hasForcedWin(state, player, depth, memo = new Map()) {
    if (state.phase === "over")
    ...
    if (!next.length)
        return false;
    const result = state.turn === player
...
}
{
    ...
    assert.deepEqual(resolution.state.probes.red, { r: 4, c: 2 }, "回収対象のメテオは、探査機移動中にはまだ障害物である");
    assert.equal(resolution.state.meteors.some((meteor) => meteor.id === 1), false, "探査機移動の判定後にメテオが回収される");
...
for (const first of ["red", "blue"]) {
    ...
    assert.equal(hasForcedWin(opening, first, 6), false, `${first}に序盤6アクション以内の強制勝利がない`);
}
{
...
}
{
    ...
    assert.deepEqual(blast.probes.red, { r: 6, c: 6 }, "BOOSTER中は小メテオの爆風を2マス受ける");
}
{
    const item = initialGameState(15, "red", 2, false, 0, [], "item");
    item.turnCount = 2;
    ...
    item.boosterMoves.red = 2;
    const blast = applyMeteor(item, { r: 9, c: 6 }, "small").state;
    assert.deepEqual(blast.probes.red, { r: 7, c: 6 }, "自分の爆風はBOOSTER中でも通常距離");
}
