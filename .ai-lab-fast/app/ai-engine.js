import { activePlayers, applyMeteor, applyMove, applyPass, distance, legalMoves, samePos, teamOf, } from "./game-rules.js";
const centerOf = (state) => {
    const mid = Math.floor(state.size / 2);
    return { r: mid, c: mid };
};
const coreDistance = (state, player) => {
    const p = state.probes[player];
    const center = centerOf(state);
    return Math.abs(p.r - center.r) + Math.abs(p.c - center.c);
};
const allied = (state, a, b) => a === b || (state.variant === "team" && teamOf(a) === teamOf(b));
const wonBy = (state, player) => state.winner !== null &&
    state.winner !== "draw" &&
    (state.winner === player ||
...
function personality(player) {
    if (player === "red")
        return { progress: 1.06, denial: 0.99, items: 0.98, resources: 0.99 };
    if (player === "blue")
        return { progress: 0.99, denial: 1.06, items: 0.99, resources: 1.01 };
    if (player === "green")
        return { progress: 1, denial: 0.99, items: 1.08, resources: 0.99 };
...
function positionValue(state, player) {
    ...
    const progress = (p) => (span * 2 - coreDistance(state, p)) / (span * 2);
    ...
    let score = friendProgress * 620 * style.progress -
...
function placements(state) {
    if (state.phase !== "place" || state.turnCount === 0)
        return [];
    const result = [];
    const center = centerOf(state);
    const occupied = (p) => samePos(p, center) ||
        activePlayers(state).some((player) => samePos(state.probes[player], p)) ||
        state.meteors.some((meteor) => samePos(meteor, p));
    const kinds = [];
    if (state.inventory[state.turn].small > 0)
        kinds.push({ size: "small", useCapsule: false });
    if (state.inventory[state.turn].large > 0)
        kinds.push({ size: "large", useCapsule: false });
    if ((state.capsuleMeteors?.[state.turn] ?? 0) > 0)
        kinds.push({ size: "small", useCapsule: true });
...
function tacticalPlacements(state) {
...
function isImmediateWinAvailable(state, player) {
    if (state.phase === "over")
        return wonBy(state, player);
    const probe = { ...state, turn: player, phase: "move", bonusMove: false };
    for (const move of legalMoves(probe, player)) {
        const afterMove = applyMove(probe, move);
        if (wonBy(afterMove, player))
            return true;
        if (afterMove.phase === "place" && afterMove.turn === player) {
            ...
            for (const placement of tactical) {
                if (wonBy(applyPlacement(afterMove, placement), player))
                    return true;
            }
        }
    }
...
function threatPenalty(state, player) {
    if (state.phase === "over")
        return 0;
    ...
    for (const rival of rivals) {
        ...
        if (close && isImmediateWinAvailable(state, rival))
            penalty += 180_000;
    }
    return penalty;
}
function scoreResult(state, player, previous) {
    const terminal = terminalValue(state, player);
    if (terminal !== null)
        return terminal;
    ...
    if (previous) {
        for (const candidate of activePlayers(state)) {
            if (allied(state, candidate, player) &&
...
function bestPlacement(state, player) {
    const options = [];
...
function selectWithDifficulty(ranked, difficulty, random) {
    ranked.sort((a, b) => b.value - a.value);
    if (difficulty === "hard" || ranked.length === 1)
        return ranked[0];
...
export function chooseAiDecision(state, difficulty = "normal", random = Math.random) {
    if (state.phase === "over")
        return { type: "skip" };
    ...
    const selected = selectWithDifficulty(ranked, difficulty, random);
    if (!selected || selected.choice === "pass")
        return { type: "pass" };
