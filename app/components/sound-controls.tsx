...
type SoundMixerProps = { enabled: boolean; musicEnabled: boolean; masterVolume: number; bgmVolume: number; sfxVolume: number; masterLabel: string; sfxLabel: string; muteLabel: string; unmuteLabel: string; setMasterVolume: (value: number) => void; setBgmVolume: (value: number) => void; setSfxVolume: (value: number) => void; onTick: () => void; onToggle: () => void };

export function SoundMixer(props: SoundMixerProps) {
  return <div className="hud-volume"><button type="button" aria-label={props.enabled ? props.muteLabel : props.unmuteLabel} onClick={props.onToggle}>{props.enabled ? "◖))" : "◖×"}</button><div className="hud-mixer"><VolumeRange label={props.masterLabel} shortLabel="ALL" value={props.masterVolume} onChange={props.setMasterVolume} onTick={props.onTick} />{props.musicEnabled && <VolumeRange label="BGM" shortLabel="BGM" value={props.bgmVolume} onChange={props.setBgmVolume} onTick={props.onTick} />}<VolumeRange label={props.sfxLabel} shortLabel="SFX" value={props.sfxVolume} onChange={props.setSfxVolume} onTick={props.onTick} /></div></div>;
}
