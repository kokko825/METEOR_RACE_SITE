...
export default function PrivacyPage() {
  return (
    <main className="doc-page">
      ...
      <section className="doc-section"><h2>保存する情報</h2><p>ニックネーム、端末ごとの内部識別子と、それから作る短いAEQRIS企業登録番号、対戦結果、レート、ルーム参加情報、ルームチャット、お問い合わせ内容を保存することがあります。認証サービスと連携した場合に限り、識別と問い合わせ通知のためメールアドレスを取り扱う場合があります。</p></section>
      <section className="doc-section"><h2>利用目的</h2><p>オンライン対戦の同期、本人のプロフィールとレートの識別、不具合調査、お問い合わせ対応、ゲーム品質の改善に利用します。</p></section>
      <section className="doc-section"><h2>匿名の好プレーデータ</h2><p>設定がONの場合、CPUを含む対戦で勝者が行った有効性の高い局面を、AI改善の検証資料として保存することがあります。盤面、モード、難易度、行動前後の状態を保存しますが、ニックネーム、メールアドレス、チャット、AEQRIS企業登録番号、ルームコードは含めず、別の対戦と同一企業として結び付けません。保存前にサーバー側で評価を再計算し、改変された値は受け付けません。AIが自動学習するためには使用せず、設定からいつでもOFFにできます。</p></section>
      <section className="doc-section"><h2>公開範囲</h2><p>他のプレイヤーに表示するのは対戦用ニックネームなど、ゲームに必要な情報だけです。内部識別子は公開せず、設定画面やお問い合わせには短いAEQRIS企業登録番号だけを使用します。</p></section>
      <section className="doc-section"><h2>端末への保存</h2><p>設定やAEQRIS企業登録番号はブラウザのローカルストレージにも保存されます。ブラウザのデータを消去すると、端末内の情報が失われる場合があります。</p></section>
